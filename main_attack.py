...
def evaluate_adv(netWrapper, loader, history, epoch, args):
    ...
    epsilons = [[0.0, 0.0], [0, 0.012], [0.012, 0], [0.006, 0.006]] # 0.02 for tsne
    suffix_best = '_6_6.npy'
    for epsilon in epsilons:
        ...
        for i, batch_data in enumerate(loader):
            ...
            if args.attack_type == "fsgm":
                ...
                preds, feat_v, feat_a = netWrapper(frame, audio)
                netWrapper.zero_grad()
                err = criterion(preds, gt) # +  F.cosine_similarity(feat_v, feat_a, 1).mean() #0.8 for kinetics
                err.backward()
                ...
                for t in range(10):
                    ...
                    preds_iter, feat_v, feat_a = netWrapper(frame_adv, audio_adv)
                    netWrapper.zero_grad()
                    err = criterion(preds_iter, gt) #+ 0.8* F.cosine_similarity(feat_v, feat_a, 1).mean()
                    err.backward()
...
def compute_coef(D, X):
    A = torch.mm(torch.mm(torch.inverse(torch.mm(D.t(), D)+1.0*torch.eye(1024).to(X.device)), D.t()), X)
    return A

def compute_dic(A, X):
    D = torch.mm(torch.mm(X, A.t()), torch.inverse(torch.mm(A, A.t())))
    return D
...
def main(args):
    ...
    for epoch in range(1, args.num_epoch + 1):
        feats_v, feats_a = train(netWrapper, loader_train, optimizer, history, epoch, args)
        ...
        D_v = torch.stack(feats_v, dim=0).view(-1, feats_v[0].size(-1)).detach().permute(1, 0)
        D_a = torch.stack(feats_a, dim=0).view(-1, feats_v[0].size(-1)).detach().permute(1, 0)
        print(D_v.size())
        suffix_best = 'best.pth'
        torch.save(D_v,
                   '{}/Dv_{}'.format(args.ckpt, suffix_best))
        torch.save(D_a,
                   '{}/Da_{}'.format(args.ckpt, suffix_best))
    print('{}/Dv_{}'.format(args.ckpt, suffix_best))
